...
void ModelRoutine::updateSpAgentOutput( const VIdx& vIdx, const SpAgent& spAgent, REAL& color, Vector<REAL>& v_extraScalar, Vector<VReal>& v_extraVector ) {
	/* MODEL START */
        ...
        // print State !!!! This needs to be changes
        S32 NumGenes = MAX_NUM_NODES;
        S32 Aut = v_temp[ NumGenes - 3 ];
        S32 Apo = v_temp[ NumGenes - 2 ];
        S32 Pro = v_temp[ NumGenes - 1 ];
        S32 State  = Aut*2*2 + Apo*2 + Pro ;
        v_extraScalar[ 0 ] = State ;
